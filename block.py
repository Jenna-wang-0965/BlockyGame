...
#TODO: IMPLEMENT THIS!!!
    def smash(self) -> bool:
        ...
        """
        if self.smashable():
            all_children_pos = self._children_positions()
            child_size = self._child_size()
            level = self.level + 1
            depth = self.max_depth
            upper_r = Block(all_children_pos[0], child_size, \
                            random.choice(COLOUR_LIST), level, depth)
            upper_l = Block(all_children_pos[1], child_size, \
                            random.choice(COLOUR_LIST), level, depth)
            lower_l = Block(all_children_pos[2], child_size, \
                            random.choice(COLOUR_LIST), level, depth)
            lower_r = Block(all_children_pos[3], child_size, \
                            random.choice(COLOUR_LIST), level, depth)
            self.children = [upper_r, upper_l, lower_l, lower_r]
            return True
        return False
...
# TODO: IMPLEMENT THIS!!!
    def swap(self, direction: int) -> bool:
        ...
        """
        if direction == 1 and len(self.children) == 4:
            upper_r_1 = self.children[0]
            upper_l_2 = self.children[1]
            lower_l_3 = self.children[2]
            lower_r_4 = self.children[3]
            self.children = [lower_r_4, lower_l_3, upper_l_2, upper_r_1]
            return True
        if direction == 0 and len(self.children) == 4:
            upper_r_1 = self.children[0]
            upper_l_2 = self.children[1]
            lower_l_3 = self.children[2]
            lower_r_4 = self.children[3]
            self.children = [upper_l_2, upper_r_1, lower_r_4, lower_l_3]
            return True
        return False
...
# TODO: IMPLEMENT THIS!!!
    def rotate(self, direction: int) -> bool:
        ...
        """
        if direction == 1 and len(self.children) == 4:#rotate clockwise
            upper_r_1 = self.children[0]
            upper_l_2 = self.children[1]
            lower_l_3 = self.children[2]
            lower_r_4 = self.children[3]
            self.children = [upper_l_2, lower_l_3, lower_r_4, upper_r_1]
            return True
        if direction == 3 and len(self.children) == 4:#rotate counter-clockwise
            upper_r_1 = self.children[0]
            upper_l_2 = self.children[1]
            lower_l_3 = self.children[2]
            lower_r_4 = self.children[3]
            self.children = [lower_r_4, upper_r_1, upper_l_2, lower_l_3]
            return True
        return False
 ...
 # TODO: IMPLEMENT THIS!!!
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if self.position == self.max_depth and self.colour != colour \
